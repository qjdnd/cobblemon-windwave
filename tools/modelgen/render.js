...
// bounding boxes for a list of [anim, time] poses
async function bounds(model, poses) {
  const browser = await getBrowser();
  const page = await browser.newPage();
  await page.route('http://local/**', async (route) => {
    const url = new URL(route.request().url());
  ...
  await page.waitForFunction(() => window.ready === true);
  const out = [];
  for (const [anim, time] of poses) out.push(await page.evaluate(([m, a, t]) => window.bounds(m, a, t), [model, anim, time]));
  await page.close();
  return out;
}

module.exports = { render, bounds, close: async () => { if (browserP) (await browserP).close(); browserP = null; } };
